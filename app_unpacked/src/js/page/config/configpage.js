...
class ConfigOptionPage {
  ...
  constructor(container, mainPage) {
    ...
    this.setValueVersion = 0;
    this.setValueQueue = Promise.resolve();
    this.onBackClick = () => { this.hide(); };
  }
  show() {
  ...
  setValue(value) {
    const version = ++this.setValueVersion;
    const option = this.configOption;
    const run = () => option?.setConfig(value);
    const result = this.setValueQueue.then(run, run);
    this.setValueQueue = result.catch(() => null);
    return result.then(() => version === this.setValueVersion);
  }
