...
export default class IndexContentsPage extends IndexSubPage {
  ...
  onActivate() {
    ...
    if (!this.fakeScrollBound) {
      this.fakeListElement.addEventListener('click', this.onFakeClick);
      this.fakeListElement.addEventListener('scroll', this.onFakeScroll);
      this.fakeScrollBound = true;
    }
  }
  onInactivate() {
    if (this.recoverRealTimeout) clearTimeout(this.recoverRealTimeout);
    this.recoverRealTimeout = 0;
  ...
  updateListRender() {
    ...
    if (shouldRenderReal && !this.renderReal) {
      ...
      if (this.recoverRealTimeout) clearTimeout(this.recoverRealTimeout);
      this.recoverRealTimeout = setTimeout(() => {
        this.recoverRealTimeout = 0;
  ...
  updateFakeList() {
    if (!this.fakeListUl || !this.listElement) return;
    this.fakeListUl.innerHTML = '';
    const container = this.listElement;
    const listItems = Array.from(container.querySelectorAll('li'));
    if (!listItems.length) return;
    const first = listItems.findIndex(item => item.offsetTop + item.clientHeight >= container.scrollTop);
    const start = first < 0 ? listItems.length - 1 : first;
    let end = start;
    for (; end < listItems.length && listItems[end].offsetTop <= container.scrollTop + container.clientHeight; end++) {
      const clone = listItems[end].cloneNode(true);
      clone.querySelector('.list-item-container')?.setAttribute('data-list-index', end);
      this.fakeListUl.appendChild(clone);
    }
    const firstItem = listItems[start];
    const lastItem = listItems[Math.max(start, end - 1)];
    this.fakeListUl.style.paddingTop = firstItem.offsetTop + 'px';
    this.fakeListUl.style.paddingBottom = Math.max(0, container.scrollHeight - lastItem.offsetTop - lastItem.clientHeight) + 'px';
  }
}
