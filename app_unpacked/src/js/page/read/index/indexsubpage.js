...
export default class IndexSubPage extends ReadSubPage {
  ...
  constructor(container, tabItem, index, indexPage, readPage) {
    ...
    this.onTabClick = () => this.indexPage.setCurrentSubPage(this);
    this.onBackClick = () => this.indexPage.hide();
    this.onKeydown = event => event.stopPropagation();
    this.highlightRaf = 0;
    this.pageButtonBound = false;
  }
  updateListRender() {
  ...
  onFirstActivate() {
    ...
    this.tabItem.addEventListener('click', this.onTabClick);
  ...
  onActivate() {
    ...
    if (!this.pageButtonBound) {
      this.pageButton.addEventListener('click', this.pageButtonAction);
      this.pageButtonBound = true;
    }
  }
  onInactivate() {
    if (this.highlightRaf) cancelAnimationFrame(this.highlightRaf);
    this.highlightRaf = 0;
    if (this.itemList) this.itemList.dispatch();
    this.itemList = null;
  }
  initUpdatePage() {
  ...
  onItemClick(item) {
    this.readPage.setCursor(item.cursor, { resetRender: false });
  ...
  listUpdated() {
    ...
    this.highlightRaf = requestAnimationFrame(() => {
      this.highlightRaf = 0;
      if (this.itemList) this.updateCurrentHighlight('start');
    });
  }
  refreshList() {
