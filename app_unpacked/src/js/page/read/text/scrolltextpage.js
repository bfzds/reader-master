...
export default class ScrollTextPage extends TextPage {
  ...
  createContainer() {
    ...
    listener.onTouch(({ grid }) => {
      ...
      if (action === 'prev') {
        this.pageUp({ resetRender: false });
      } else if (action === 'next') {
        this.pageDown({ resetRender: false });
      } else if (action === 'menu') {
        this.readPage.showControlPage();
  ...
  onScroll() {
    ...
    window.requestAnimationFrame(() => {
      if (!this.isCurrent) return;
      this.onScrollScheduled = false;
      this.updatePage({ resetRender: false, debug: true });
      setTimeout(() => {
        if (!this.isCurrent || thisScrollEvent !== this.lastScrollEvent) return;
        this.onScrollDone({ resetRender: false });
  ...
  keyboardEvents(event) {
    ...
    if (['ArrowUp', 'ArrowDown'].includes(event.code) && this.autoScrollRunning) {
      const pow = event.code === 'ArrowUp' ? -1 : 1;
      const speedFactor = this.autoScrollSpeedFactor * 1.1 ** pow;
      this.autoScrollUpdate({ speedFactor });
    } else if (['PageUp', 'PageDown', 'ArrowUp', 'ArrowDown'].includes(event.code)) {
  ...
  pageUp(config) {
    ...
    const autoRunning = this.scrollToBusy || this.autoScrollBusy();
  ...
  clearPage() {
    this.scrollActive = false;
    this.trunks.splice(0).forEach(trunk => {
      trunk.element.remove();
      this.releaseResourceScope(trunk.resourceScope);
    });
    this.setScrollTop(0);
  }
  /**
  ...
  renderParagraph(text, trunk, { start, end, last, heading, trucked }) {
    ...
    this.renderInlineContent(paragraph, text, trunk.resourceScope);
  ...
  renderTrunk(start, end, position) {
    ...
    /** @type {TrunkInfo} */
    const trunk = { element, paragraphs: [], start, end, resourceScope: this.createResourceScope() };
    ...
    for (let pos = start, prev = start; pos < end; prev = pos) {
      if (++loopCount > maxParagraphs) {
        // Force break to prevent UI freeze
        end = prev;
        trunk.end = end;
        element.dataset.end = end;
        if (trunk.end === content.length) element.classList.add('trunk-last');
        else element.classList.remove('trunk-last');
        break;
      }
    ...
    if (position === 'first') {
      if (this.trunks.length) {
        ...
        if (oldFirstParagraph && firstLastParagraph) {
      ...
      if (this.trunks.length) {
        ...
        if (oldLastParagraph && lastFirstParagraph) {
  ...
  removeTrunk(position) {
    ...
    if (trunk.next) {
      ...
      trunk.next = null;
      next.prev = null;
      const lastParagraph = trunk.paragraphs[trunk.paragraphs.length - 1];
      if (lastParagraph) lastParagraph.next = null;
      if (next.paragraphs[0]) next.paragraphs[0].prev = null;
    }
    if (trunk.prev) {
      const prev = trunk.prev;
      trunk.prev = null;
      prev.next = null;
      if (trunk.paragraphs[0]) trunk.paragraphs[0].prev = null;
      const lastParagraph = prev.paragraphs[prev.paragraphs.length - 1];
      if (lastParagraph) lastParagraph.next = null;
    }
    if (position === 'first') {
      this.updateTrunkOffsetTop();
  ...
  getTrunkEndPosition(start) {
    ...
    // Safety check: limit maximum lines per trunk to prevent freeze
    let safeEnd = start;
    const maxLines = 300;
    for (let i = 0; i < maxLines; i++) {
      safeEnd = content.indexOf('\n', safeEnd);
      if (safeEnd === -1) {
        safeEnd = content.length;
        break;
      }
      safeEnd++; // Skip the newline char
      if (safeEnd >= content.length) break;
    }
  ...
  getTrunkStartPosition(end) {
    ...
    // Safety check: limit maximum lines per trunk
    let safeStart = end;
    const maxLines = 300;
    for (let i = 0; i < maxLines; i++) {
      if (safeStart <= 0) {
      ...
      safeStart = content.lastIndexOf('\n', safeStart - 1);
    }
    if (safeStart !== -1) safeStart++; // Move to char after \n
    if (start < safeStart) start = safeStart;

    // Ensure start is strictly less than end to avoid infinite loops with empty trunks
    if (start >= end && end > 0) {
      console.warn('ScrollTextPage getTrunkStartPosition corrected', start, end);
    ...
    while (true) {
      if (++loopCount > 1000) {
        console.warn('ScrollTextPage getTrunkStartPosition loop limit exceeded');
        return start;
      }
      const ref = this.getTrunkEndPosition(start);
      if (ref <= start) {
        console.warn('ScrollTextPage getTrunkStartPosition stuck');
        return start;
      }
  ...
  getTextPosition(paragraph, index, type) {
    ...
    const boundary = this.getTextBoundary(element, index);
    if (!boundary) return element.getBoundingClientRect().bottom;
    const range = document.createRange();
    range.setStart(boundary.node, boundary.offset);
    range.setEnd(boundary.node, Math.min(boundary.offset + 1, boundary.node.nodeValue.length));
    const rects = Array.from(range.getClientRects());
    const rect = rects.find(rect => rect.width * rect.height) ?? rects[0];
    if (!rect) return element.getBoundingClientRect()[type];
  ...
  updatePagePrev() {
    ...
    if (!trunks?.length || !this.currentTrunk) return null;
    ...
    if (prevHeight < minimumHeight) {
      let loopCount = 0;
      do {
        if (++loopCount > 100) {
          console.warn('ScrollTextPage updatePagePrev loop limit exceeded');
          break;
        }
        const end = trunks[0].start;
        if (end === 0) break;
        const trunk = this.renderTrunkEndsWith(end, 'first');
        if (trunk.height <= 0 && !trunk.paragraphs.length) {
          // console.warn('ScrollTextPage updatePagePrev trunk height 0');
          if (trunk.start === trunk.end) {
             this.removeTrunk('first');
             break;
          }
        }
  ...
  updatePageNext() {
    ...
    if (nextHeight < minimumHeight) {
      ...
      do {
        if (++loopCount > 100) {
          console.warn('ScrollTextPage updatePageNext loop limit exceeded');
          break;
        }
        ...
        if (trunk.height <= 0 && !trunk.paragraphs.length) {
          // console.warn('ScrollTextPage updatePageNext trunk height 0');
          if (trunk.start === trunk.end) {
             this.removeTrunk('last');
             break;
          }
        }
  ...
  updatePageMeta() {
    ...
    const cursor = this.currentRenderCursor ?? 0;
  ...
  async updatePage(config) {
    if (!this.isCurrent || !this.readBodyElement) return;
    this.updatePageRender();
  ...
  highlightChars(start, length, textHighlight = false, depth = 0) {
    ...
    const resetAndRetry = pageDown => {
      ...
      if (pageDown) this.pageDown({ resetRender: false });
      else this.readPage.setCursor(start, { resetRender: true });
    ...
    if (textHighlight) {
      return this.markTextHighlight(paragraph.element, start - paragraph.start, Math.min(length, paragraph.last - start));
    }
    ...
    const range = this.getTextRange(
      ...
      Math.min(length, paragraph.last - start),
    );
    ...
    if (!firstRect) return highlightSpanList;
    if (!this.scrollToBusy) {
  ...
  highlightSearchMatches(reg, start, length, depth = 0) {
    ...
    this.clearSearchTextHighlight();
    const pageStart = this.getPageStartPosition();
    const pageEnd = this.getPageEndPosition();
    if (!pageStart || !pageEnd) return result;
    const visibleStart = pageStart.cursor;
    const visibleEnd = pageEnd.cursor;
    const marks = [];
    this.trunks.forEach(trunk => {
      trunk.paragraphs.forEach(paragraph => {
        if (paragraph.end < visibleStart || paragraph.start > visibleEnd) return;
        const text = paragraph.element.textContent;
        marks.push(...this.markSearchMatches(paragraph.element, text, reg, visibleStart - paragraph.start, visibleEnd - paragraph.start));
      });
    });
