...
export default class Menu {
  ...
  keyboardHandler(event) {
    if (!this.bindingValue || !this.resolve) return;
  ...
  clickHandler(event) {
    ...
    if (!(target instanceof Element) || !this.bindingValue) return;
    const item = target.closest('.screen-option-item');
    if (!item || !this.bindingValue.has(item)) return;
    this.resolve(this.bindingValue.get(item));
  }
  renderMenu() {
    /** @type {Map<HTMLElement, ValueType>} */
    this.bindingValue = new Map();
    this.groups.filter(group => group?.length).forEach(group => {
  ...
  focusMenu() {
    this.menuElement.querySelector('.screen-option-item')?.focus();
  }
  open() {
    if (Menu.activeMenu || !this.groups.some(group => group?.length)) return false;
  ...
  unbind() {
    ...
    if (Menu.activeMenu === this && this.resolve) this.resolve();
    this.button = null;
    this.callback = null;
  }
}
