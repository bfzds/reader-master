...
export default class ItemList {
  ...
  constructor(container, config) {
    ...
    this.pendingRemoveTimers = new Set();
    ...
    if (typeof this.onItemClick === 'function') {
      ...
      this.listElement.addEventListener('click', this.boundClick);
    }
    ...
    if (this.hasRemove) {
      ...
      this.listElement.addEventListener('contextmenu', this.boundContextMenu);
    }
    ...
    this.listElement.addEventListener('scroll', this.boundScroll);
  }
  renderItem(item, index) {
    ...
    if (this.hasRemove && this.mayRemove(item, index)) {
      ...
      const hideRemoveListener = () => {
        if (showDelete && !showDeleteMove) slideDelete('hide');
      };
      this.removeHideListeners.push(hideRemoveListener);
      this.listElement.addEventListener('__hide_remove', hideRemoveListener);
  ...
  dispatch() {
    ...
    this.listElement.removeEventListener('click', this.boundClick);
    this.listElement.removeEventListener('contextmenu', this.boundContextMenu);
    this.listElement.removeEventListener('scroll', this.boundScroll);
    this.removeHideListeners.forEach(listener => {
      this.listElement.removeEventListener('__hide_remove', listener);
    });
    this.removeHideListeners = [];
  ...
  scrollIntoView(index, options) {
    const element = this.elements.get(index);
    if (element) element.scrollIntoView(options ?? { block: 'nearest' });
  }
  clearSelectItem() {
    if (!this.selectable) return;
  ...
  renderRemoveItem(element) {
    ...
    window.requestAnimationFrame(() => {
      ...
      const timer = setTimeout(() => {
        this.pendingRemoveTimers.delete(timer);
        element.remove();
      }, 200);
      this.pendingRemoveTimers.add(timer);
    });
  }
  indexMapping(f) {
  ...
  removeItem(index) {
    if (index < 0 || index >= this.list.length || !this.elements.has(index)) return;
  ...
  insertItem(item, index) {
    ...
    const li = this.renderItem(item, index);
    if (next) next.parentNode.insertBefore(li, next);
    else this.listElement.appendChild(li);
  }
