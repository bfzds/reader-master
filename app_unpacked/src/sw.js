...
const version = /* VERSION */"20260805-e409b9399f43"/* VERSION */; // eslint-disable-line-line quotes
    ...
    if (url.href === new URL('?version', location.href).href) {
      ...
      event.respondWith((async () => {
        ...
        try {
          ...
          // Network failed (dev server down / offline). Try fallback cache; otherwise return 503.
          const fallback = await caches.match('./');
          return fallback || new Response('Failed to fetch', { status: 503 });
        }
      })());
    }
    return;
