...
// A fallback segment never exceeds this size unless a shorter paragraph boundary is available.
const FALLBACK_MAX_CHARS = 40000;
// Prefer a natural paragraph boundary so short paragraphs still produce a navigable TOC.
const FALLBACK_PARAGRAPHS = 80;
...
const recognizeContents = function (article, articleContext = {}) {
  ...
  cursor = 0;
  article.split('\n').forEach(line => {
    const mismatchResult = mismatchList.map(r => {
      return matchContentLine(r, line);
    });
    if (line.length <= MAX_TITLE_LENGTH) {
      regexList.forEach((matchReg, index) => {
        if (matchContentLine(matchReg, line)) {
          contents[index].push({
            title: line.trim(),
            cursor,
          });
...
const fallbackTemplateList = [
  '/^\\s*(?:第[零〇一二三四五六七八九十百千万两0-9]+[章节回篇话](?:\\s*[:：、.．-]?\\s*[^\\n]{0,80})?|(?:楔子|序章|前言|引言|后记|尾声|番外)(?:\\s*[:：、.．-]?\\s*[^\\n]{0,80})?)\\s*$/u',
  '/^\\s*(?:第[零〇一二三四五六七八九十百千万两0-9]+卷|卷[零〇一二三四五六七八九十百千万两0-9]+)(?:\\s*[:：、.．-]?\\s*[^\\n]{0,80})?\\s*$/u',
  '/^\\s*(?:(?:chapter|chap\\.?|part|book|volume|vol\\.?)\\s*(?:\\d+|[ivxlcdm]+)(?:\\s*[:：.\\-]\\s*[^\\n]{0,80})?|(?:prologue|epilogue|preface|afterword)(?:\\s*[:：.\\-]\\s*[^\\n]{0,80})?)\\s*$/iu',
  '/^\\s*(?:\\d+(?:\\.\\d+)*|[一二三四五六七八九十]+)[、.．:：]\\s*[^\\n]{1,80}\\s*$/u',
];

const recognizeContentsByTemplate = function (article, template) {
  const matchReg = parseContentTemplate(template);
  const items = [];
  let cursor = 0;
  article.split('\n').forEach(line => {
    if (line.length <= MAX_TITLE_LENGTH && matchContentLine(matchReg, line)) {
      items.push({ title: line.trim(), cursor });
    }
    cursor += line.length + 1;
  });
  return items;
...
const recognizeContentsByFallbackTemplate = function (article) {
  for (const template of fallbackTemplateList) {
    const items = recognizeContentsByTemplate(article, template);
    const titleSet = new Set(items.map(item => item.title));
    if (items.length >= 2 && titleSet.size >= 2) {
...
const findFallbackSectionEnd = function (article, start) {
  const hardEnd = Math.min(start + FALLBACK_MAX_CHARS, article.length);
  let paragraphCount = 0;
  let lineStart = start;
  for (let index = start; index < hardEnd; index++) {
    if (article[index] !== '\n') continue;
    if (article.slice(lineStart, index).trim()) {
      paragraphCount++;
      if (paragraphCount >= FALLBACK_PARAGRAPHS) return index + 1;
    }
    lineStart = index + 1;
  }
  if (hardEnd === article.length) return hardEnd;

  const readableEnd = article.slice(start, hardEnd);
  const punctuationIndex = Math.max(
    readableEnd.lastIndexOf('\n'),
    readableEnd.lastIndexOf('。'),
    readableEnd.lastIndexOf('！'),
    readableEnd.lastIndexOf('？'),
    readableEnd.lastIndexOf('!'),
  ...
  return punctuationIndex >= 0 ? start + punctuationIndex + 1 : hardEnd;
};

const createFallbackContents = function (article) {
  const items = [];
  let start = 0;
  let section = 1;
  while (start < article.length) {
    const end = findFallbackSectionEnd(article, start);
    if (end >= article.length) break;
    section++;
    items.push({ title: `第 ${section} 段`, cursor: end });
    start = end;
  }
  return { items, template: '' };
};
 ...
 * @returns {RegExp}
 */
const useWildcardForContent = function (template) {
 ...
 * @returns {boolean}
 */
const matchContentLine = function (matchReg, line) {
...
const parseContentTemplate = function (template) {
  ...
  return useWildcardForContent(template);
};
...
self.addEventListener('message', event => {
  ...
  try {
    const result = recognizeContents(fileContent) ||
      recognizeContentsByFallbackTemplate(fileContent) ||
      createFallbackContents(fileContent);
    postMessage(result);
  } catch (e) {
    console.error(e);
    postMessage(null);
