...
Expected: 确认 `.git` 是否为空壳、提交身份是否可用；如果全局和本地身份都缺失，停止基线任务并要求用户提供姓名和邮箱，不执行 `git init` 或首 commit。

- [ ] **Step 2: 确认远程托管平台**

检查用户要使用 GitHub、GitLab、Gitea 还是仅本地 Git。只有确认 GitHub 后才创建 `.github/workflows/verify.yml`；其他平台使用相同命令，但放入对应 CI 配置。

- [ ] **Step 3: 明确首个基线 commit 的边界**

首 commit 只包含源码、配置、文档和测试，不包含构建产物。commit message 使用：`chore: establish source baseline with known mojibake and retired speech remnants`，明确该基线仍含已知乱码和语音残留。

commit 前先 `git add -A` 再 `git status --short` 逐项预览，确认几百 MB 的 `node_modules`、`dist`、`target*` 全部被 ignore、没有误加进暂存区，再执行 commit。

- [ ] **Step 4: 补充构建产物忽略规则**

在 `.gitignore` 增加 `/src-tauri/target-*/`，保留已有 `/src-tauri/target/`、`/artifacts/` 和 `node_modules` 规则。

- [ ] **Step 5: 编写并验证统一 Node 测试入口**

`scripts/test-all.mjs` 使用 `readdir` 找到 `scripts/test-*.mjs`，按文件名排序，用 `spawnSync(process.execPath, ['--test', file])` 执行；任一失败时返回非零退出码。

Run: `node scripts/test-all.mjs`

Expected: 当前 8 个 Node 测试文件都被执行，并显示失败文件名。

- [ ] **Step 6: 注册 `npm test`，再按托管平台补 CI**

在 `package.json` 增加 `"test": "node scripts/test-all.mjs"`。GitHub 场景下，工作流依次运行 `npm ci`、`npm test`、`cargo test --manifest-path src-tauri/Cargo.toml` 和 JS `node --check`；非 GitHub 场景写入对应平台。

- [ ] **Step 7: 更新文档并验证基线**

Run: `npm.cmd test; cargo test --manifest-path src-tauri/Cargo.toml`

Expected: Node 和 Rust 测试通过；`docs/testing.md` 记录首 commit 的已知问题和新的统一入口。
...
- 删除语音占位模块前，`rg -n "speech|readspeech|configSpeech|buttonSpeech" app_unpacked/src` 不得再发现真实运行时 import。
...
Expected: 语法和现有测试通过；剩余语音命中只有明确保留的历史说明。
...
- `sw.js:10` 使用 `YYYYMMDD-<resource-hash>` 格式；清单内容变化必须改变 hash，触发旧缓存淘汰。
...
脚本读取 `sw.js` 的 `resourceList`，计算清单 hash，按当前日期生成版本，并只更新 `/* VERSION */.../* VERSION */` 标记之间的值。
...
使用单个可取消的 `tokio::time::sleep(Duration::from_millis(250))` 任务；连续事件只替换待写入的逻辑尺寸，计时器到期后调用现有 `write_app_config()`。
...
**Interfaces:**
- `config/csp-dev.txt` 和 `config/csp-prod.txt` 是公共配置源；Node 和 Rust 从公共配置读取，`tauri.conf.json` 保留 Tauri 必需的字符串并由测试断言一致。
...
- 小于 `1 MiB` 的正文保留原路径；超过阈值的正文拆成 `1 MiB` 分块，`content` store 保存描述信息和资源元数据。
...
**Interfaces:**
- updater 插件配置、更新 endpoint 和发布工作流先独立完成，不被 Windows 代码签名证书阻塞。
- updater 包签名和 Windows Authenticode 是两条不同链路，分别配置、分别验收。
- NSIS 安装包继续保留，作为首次安装和手动升级 fallback。

- [ ] **Step 1: 接入 updater 插件和权限配置**

添加与 Tauri 2 匹配的 updater 插件，配置更新 endpoint、公开配置和应用端检查/安装入口；不把私钥写入仓库。
...
Run: `rg -n -i "private.?key|password|secret" src-tauri docs .github; npm.cmd test; cargo test --manifest-path src-tauri/Cargo.toml`
