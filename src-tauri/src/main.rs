...
struct StaticServerState { _server: shell::StaticServerHandle }
...
fn load_import_folder_registry(app: &AppHandle) -> ImportFolderRegistry {
  ...
  registry.folders.retain(|_, path| {
    std::fs::canonicalize(path).map(|canonical| canonical.is_dir()).unwrap_or(false)
  });
  registry.selected_id = registry.selected_id.filter(|id| registry.folders.contains_key(id));
  registry
}

fn save_import_folder_registry(app: &AppHandle, registry: &ImportFolderRegistry) -> Result<(), String> {
...
fn is_symlink(path: &Path) -> Result<bool, String> {
  Ok(std::fs::symlink_metadata(path).map_err(|error| error.to_string())?.file_type().is_symlink())
}
...
fn authorized_folder(state: &ImportFolderState, folder_id: &str) -> Result<PathBuf, String> {
  let registry = state.0.lock().map_err(|_| "导入文件夹状态不可用".to_string())?;
  let root = registry.folders.get(folder_id).ok_or("导入文件夹未授权，请重新选择文件夹")?;
  let canonical = std::fs::canonicalize(root).map_err(|_| "导入文件夹已不可用，请重新选择文件夹".to_string())?;
...
async fn save_config_file(name: String, content: String) -> Result<bool, String> {
  ...
  tokio::fs::write(path, content.as_bytes()).await.map_err(|error| error.to_string())?;
  Ok(true)
}
...
async fn read_dropped_file(path: String) -> Result<FolderFileEntry, String> {
  ...
  let metadata = tokio::fs::metadata(&canonical).await.map_err(|error| error.to_string())?;
  if metadata.len() > MAX_IMPORT_FILE_BYTES {
    return Err("书籍文件过大，拒绝读取".to_string());
  }
  let bytes = tokio::fs::read(&canonical).await.map_err(|error| error.to_string())?;
  let modified = metadata
    ...
    .unwrap_or(0);
  Ok(FolderFileEntry {
    name: canonical.file_name().and_then(|value| value.to_str()).unwrap_or("book.txt").to_string(),
    size: metadata.len(),
...
#[tauri::command]
async fn pick_import_folder(app: AppHandle, state: State<'_, ImportFolderState>) -> Result<Option<PickFolderResult>, String> {
  ...
  let mut registry = state.0.lock().map_err(|_| "导入文件夹状态不可用".to_string())?;
...
#[tauri::command]
async fn get_import_folder_selection(state: State<'_, ImportFolderState>) -> Result<Option<PickFolderResult>, String> {
  let registry = state.0.lock().map_err(|_| "导入文件夹状态不可用".to_string())?;
  ...
  Ok(Some(PickFolderResult { id: id.clone(), name }))
}

#[tauri::command]
async fn save_file_to_folder(state: State<'_, ImportFolderState>, folder_id: String, name: String, bytes: Vec<u8>) -> Result<bool, String> {
  ...
  tokio::fs::write(target, bytes).await.map_err(|error| error.to_string())?;
  Ok(true)
}

#[tauri::command]
async fn list_import_folder_books(state: State<'_, ImportFolderState>, folder_id: String) -> Result<Vec<FolderFileEntry>, String> {
  let resolved = authorized_folder(&state, &folder_id)?;
  let mut reader = tokio::fs::read_dir(resolved).await.map_err(|error| error.to_string())?;
...
#[tauri::command]
async fn read_file_in_folder(state: State<'_, ImportFolderState>, folder_id: String, name: String) -> Result<FolderFileEntry, String> {
  let resolved = authorized_folder(&state, &folder_id)?;
  let filename = sanitize_filename(&name);
  let target = authorized_file_path(&resolved, &filename)?;
  let metadata = tokio::fs::metadata(&target).await.map_err(|error| error.to_string())?;
  if metadata.len() > MAX_IMPORT_FILE_BYTES {
    return Err("书籍文件过大，拒绝读取".to_string());
  }
  let bytes = tokio::fs::read(&target).await.map_err(|error| error.to_string())?;
  let modified = metadata
    .modified()
    .ok()
    .and_then(|time| time.duration_since(std::time::UNIX_EPOCH).ok())
...
#[tauri::command]
async fn delete_file_in_folder(state: State<'_, ImportFolderState>, folder_id: String, name: String) -> Result<bool, String> {
  let resolved = authorized_folder(&state, &folder_id)?;
  let target = writable_file_path(&resolved, &name)?;
...
#[tauri::command]
async fn set_window_size(window: WebviewWindow, app: AppHandle, width: u32, height: u32) -> Result<bool, String> {
...
fn create_main_window(app: &AppHandle, config: &AppConfig) -> Result<WebviewWindow, String> {
  ...
  let window = tauri::WebviewWindowBuilder::new(app, "main", url)
    .title("tReader")
    .inner_size(config.window_size.width as f64, config.window_size.height as f64)
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      if !cfg!(debug_assertions) {
        ...
        let static_server = tauri::async_runtime::block_on(shell::spawn_static_server(root_dir, HOST, PORT))
          .map_err(|error| {
            let message = format!(
        ...
        app.manage(StaticServerState { _server: static_server });
      }
      let registry = load_import_folder_registry(app.handle());
      *app.state::<ImportFolderState>().0.lock().map_err(|_| "导入文件夹状态不可用")? = registry;
